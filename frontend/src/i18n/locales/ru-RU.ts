export default {
  ...
  commandPalette: {
    ...
    quick: {
      ...
      workflows: 'Открыть рабочие процессы',
    ...
    workflowCreate: {
      steps: {
        templates: {
          ...
          desc: 'Шаблон — это уже соединённая схема. Выберите подходящий и замените отмеченные места на свою базу знаний или инструмент.'
        },
        canvas: {
          title: 'Это и есть схема',
          desc: 'Каждый блок — один шаг, а стрелки показывают порядок выполнения. Блоки можно перетаскивать: меняется только картинка, а не порядок работы.'
        },
        palette: {
          title: 'Добавьте шаг',
          desc: 'Нажмите на шаг слева, чтобы добавить его на холст, затем протяните линию от предыдущего блока.'
        },
        inspector: {
          title: 'Заполните здесь',
          desc: 'Для выбранного узла нужные поля появятся справа. Поля с красной звёздочкой обязательны, без них сохранить не получится.'
        }
      }
    },
    agentList: {
  ...
  workflow: {
    ...
    delete: {
      ...
      confirmMessage: 'Вы уверены, что хотите удалить рабочий процесс «{name}»? Это действие необратимо.',
      confirmButton: 'Подтвердить удаление',
    },
    detail: {
      title: 'Сведения о рабочем процессе',
    },
    empty: {
      title: 'Нет рабочих процессов',
      description: 'Рабочие процессы отлично подходят для структурированных задач. Начните с готовых шаблонов.',
      sharedTitle: 'Нет общих процессов',
      sharedDescription: 'Процессы, к которым вам предоставили доступ, появятся здесь',
      favoritesTitle: 'Нет избранных процессов',
      favoritesDescription: 'Добавляйте процессы в избранное для быстрого доступа',
      recentsTitle: 'Нет недавних процессов',
      recentsDescription: 'Недавно использованные процессы появятся здесь',
    },
  }
}
