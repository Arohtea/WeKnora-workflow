                ...
                <t-tooltip :content="item.title" placement="right" :disabled="!uiStore.sidebarCollapsed">
                    <div @click="handleMenuClick(item.path)" @mouseenter="mouseenteMenu(item.path)"
                        ...
                        <div class="menu_item-box">
                            <div class="menu_icon">
                                <img class="icon"
                                    :src="getImgSrc(item.icon == 'zhishiku' ? knowledgeIcon : item.icon == 'agent' ? agentIcon : item.icon == 'workflow' ? workflowIcon : item.icon == 'organization' ? organizationIcon : item.icon == 'logout' ? logoutIcon : item.icon == 'setting' ? settingIcon : prefixIcon)"
                                    alt="">
                            </div>
...
const isMenuItemActive = (itemPath: string): boolean => {
    ...
    switch (itemPath) {
        ...
        case 'workflows':
            return currentRoute === 'workflowList';
        case 'organizations':
...
const topMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) =>
        item.path === 'knowledge-bases' || item.path === 'agents' || item.path === 'workflows' || item.path === 'organizations' || item.path === 'creatChat'
    );
});

const bottomMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) => {
        if (item.path === 'knowledge-bases' || item.path === 'agents' || item.path === 'workflows' || item.path === 'organizations' || item.path === 'creatChat') {
            return false;
        }
        return true;
    });
...
let workflowIcon = ref('integration.svg');
...
const getIcon = (path: string) => {
    ...
    const workflowsActiveState = route.name === 'workflowList';
    ...
    // 工作流复用编排图标，与智能体保持同级入口的视觉区分。
    workflowIcon.value = workflowsActiveState ? 'integration-green.svg' : 'integration.svg';
