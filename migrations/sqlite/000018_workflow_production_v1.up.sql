...
ALTER TABLE custom_agents ADD COLUMN draft_revision INTEGER NOT NULL DEFAULT 0;
ALTER TABLE custom_agents ADD COLUMN published_version INTEGER NOT NULL DEFAULT 0;
...
CREATE TABLE IF NOT EXISTS workflow_runs (
    ...
    error_truncated BOOLEAN NOT NULL DEFAULT 0,
    usage TEXT,
...
CREATE TABLE IF NOT EXISTS workflow_run_branches (
    ...
    last_node_run_id INTEGER NOT NULL DEFAULT 0,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS workflow_run_events (
    ...
    summary_truncated BOOLEAN NOT NULL DEFAULT 0,
    error_code VARCHAR(64) NOT NULL DEFAULT '',
...
WHERE json_extract(config, '$.agent_type') = 'workflow'
  AND deleted_at IS NULL
  AND json_extract(config, '$.workflow') IS NOT NULL;
...
SELECT tenant_id,
       ...
       published_version,
       draft_revision,
       COALESCE(json_extract(config, '$.workflow'), '{}'),
       config,
       '',
